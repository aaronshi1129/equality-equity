...
const equalityBtn = document.getElementById('equality-btn');
...
function startGame() {
    ...
    // Shuffle the scenarios array to get random order
    ...
    // Add event listeners
    equalityBtn.addEventListener('click', () => checkAnswer('equality'));
    equityBtn.addEventListener('click', () => checkAnswer('equity'));
...
function showScenario() {
    if (currentScenarioIndex < scenarios.length) {
        ...
        // Record the time when the scenario is shown
        lastAnswerTime = Date.now();
    } else {
        // Game over - all scenarios completed
        endGame();
    }
}
...
function checkAnswer(answer) {
    ...
    const isCorrect = answer !== 'timeout' && answer === correctAnswer;
    ...
    if (answer === 'timeout') {
        ...
        feedbackText.textContent = `Time's up! ${scenarios[currentScenarioIndex].explanation}`;
        currentStreak = 0;
    } else {
...
function restartGame() {
    ...
    const restartBtn = document.querySelector('.game-container button:not(#next-btn):not(#equality-btn):not(#equity-btn)');
    if (restartBtn) {
        restartBtn.remove();
    }
